...
network8 = require 'network.network8'
network16 = require 'network.network16'
network32 = require 'network.network32'
...
opt = lapp[[
  ...
  --ifwbupsampleG2     (default 0)        16->32 G network
  --mode              (default 1)        train mode
  --seed               (default 1)        set the random seed
]]
...
gFile = io.open(os.date("%Y-%m-%d %H:%M:%S").."-".."configure.txt" ,"w");
gFile:write("seed:"..opt.seed.."\n")
gFile:write("coefL1:"..opt.coefL1.."\n")
gFile:write("coefL2:"..opt.coefL2.."\n")
gFile:write("K:"..opt.K.."\n")
gFile:close()
